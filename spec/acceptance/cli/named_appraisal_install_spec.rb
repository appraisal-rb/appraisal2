...
RSpec.describe "CLI", ".install with named appraisal" do
  context "when targeting a specific appraisal" do
    ...
    it "installs dependencies for only the named appraisal" do
      ...
      expect(file("gemfiles/rails-7.gemfile.lock")).to be_exists
      expect(content_of("gemfiles/rails-7.gemfile.lock")).to include("dummy (1.1.0)")
    end

    it "accepts --gem-manager option" do
      output = run "appraisal rails-7 install --gem-manager=bundler"

      expect(output).to include("bundle install --gemfile='#{file("gemfiles/rails-7.gemfile")}'")
      expect(file("gemfiles/rails-7.gemfile.lock")).to be_exists
    end
    ...
    it "accepts --jobs option with named appraisal", :parallel do
      ...
      expect(output).to include("bundle install --gemfile='#{file("gemfiles/rails-7.gemfile")}' --jobs=2")
      expect(file("gemfiles/rails-7.gemfile.lock")).to be_exists
    end
    ...
    it "does not install other appraisals" do
      ...
      expect(file("gemfiles/rails-7.gemfile.lock")).to be_exists
      expect(file("gemfiles/rails-6.gemfile.lock")).not_to be_exists
    end
  end
